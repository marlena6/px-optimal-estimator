...
def estimate_px(Np, delta_x_matrix, pix_spacing, delta_flux, m_offsets, n_offsets, mmax, nmax, nstep, kbin_est, S_fiducial, C_0_invmat, approx_fisher = False):
            ...
            for n in range(0, nmax, nstep):
                ...
                if not np.isnan(delta_I).any():
                    ...
                    for c in range(len(m2lr)):
                        delta_J = delta_flux[m2lr[c],n2lr[c]] # this is quasar J at [m2lr[c],n2lr[c]] which is sep away from quasar I at [m,n]
    ...
    if np.linalg.matrix_rank(F_alpha_beta) < len(kbin_est):
        print("Matrix is not invertible.")
        return kbin_est_centers, 0, F_alpha_beta, Lbeta, quasar_pair_counter
    else: 
        print("Matrix is supposed to be invertible.")
